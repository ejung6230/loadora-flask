...
def search_auctions_categories(item_name):
    """
    특정 아이템명을 검색하여, 해당 아이템이 포함된 모든 카테고리 코드 반환
    
    # 사용 예시
    category_codes = search_auctions_categories("혼돈")
    print(category_codes)
    """
    result_codes = set()  # 중복 방지
    ...
    try:
        ...
        if markets_match:
            ...
            if not item_name:
                ...
                if startup_state == 0:
                    response_text = f"초기 데이터 생성을 시작합니다. 잠시 후에 다시 시도해주세요."
                elif startup_state == 1:
                    response_text = f"아직 서버가 초기 데이터를 생성 중이에요. 잠시 후에 다시 시도해주세요."
